...
function submitEscalation(formData) {
  ...
  try {
    ...
    if (!/^\d-\d{13,15}$/.test(formData.caseId.trim())) return { success: false, message: 'Invalid Case ID format.' };
...
function getSchedule(dayOffset) {
  _checkRateLimit('getSchedule');
  try {
    const baseOffset = Number(dayOffset) || 0;
    const ss    = SpreadsheetApp.openById(MASTER_DB_ID);
    const sheet = ss.getSheetByName(SHEET_SUPERS);
    if (!sheet) return [];
    ...
    const targetDates = [0, 1, 2].map(function(offset) {
      ...
      d.setUTCDate(d.getUTCDate() + baseOffset + offset);
      return d;
    });
    ...
    function makeDisplayLabel(d, offset) {
      if (baseOffset === 0 && offset === 0) return 'Today';
      if (baseOffset === 0 && offset === 1) return 'Tomorrow';
    ...
    dataRange.forEach(function(row) {
      ...
      const site  = String(row[2] || '').trim().toUpperCase();
      ...
      const days = targetColIndices.map(function(tc, offset) {
        var rawVal = '';
        if (tc.colIdx >= 0) {
          // +4 offset: cols A,B,C,D are indices 0,1,2,3; col E starts at index 4
          rawVal = String(row[tc.colIdx + 4] || '').trim();
        }
        ...
        if (rawVal === '') {
          ...
          if (baseOffset === 0 && offset === 0) { // Only do real-time math for "Today"
            ...
            var yestVal = tc.colIdx > 0 ? String(row[tc.colIdx + 4 - 1] || '').trim() : '';
      ...
      result.push({ email, ldap, role, site, days });
    });
...
function claimCase(rowIdx) {
  ...
  try {
    ...
    if (String(row[13] || '').trim() !== '') {
      return { success: false, message: 'Case already claimed by someone else.' };
    }
...
function unclaimCase(rowIdx) {
  requireSME_();
  try {
    const ss       = SpreadsheetApp.openById(MASTER_DB_ID);
    const sheet    = ss.getSheetByName('Open Sup Cases');
    const smeEmail = Session.getActiveUser().getEmail();
    const handledBy = String(sheet.getRange(rowIdx, 14).getValue() || '').trim().toLowerCase();

    if (handledBy !== smeEmail.toLowerCase().trim()) {
      return { success: false, message: 'Only the SME who claimed this case can release it.' };
    }

    sheet.getRange(rowIdx, 14).setValue('');
    sheet.getRange(rowIdx, 16).setValue('');
    _log('UNCLAIM', smeEmail, 'Case released back to queue', rowIdx, '', '');

    return { success: true, message: 'Case released.' };
  } catch (e) { return { success: false, message: e.message }; }
}
...
function resolveMultipleCases(rowIdxArray, remarks, resolutionType) {
  ...
  rowIdxArray.forEach(function(rowIdx) {
    try {
      const row = sheet.getRange(rowIdx, 1, 1, 16).getValues()[0];
      const caseData = {
        submitter: String(row[1] || ''), ldap: String(row[2] || ''), caseId: String(row[3] || ''),
        symptom: String(row[4] || ''), detailedIssue: String(row[5] || ''), reason: String(row[6] || ''),
        channel: String(row[7] || ''), team: String(row[8] || ''), caseLink: String(row[9] || ''),
        timestamp: row[0] ? new Date(row[0]).toString() : '',
        resolutionTime: new Date().toString(), remarks: remarks
      };
      ...
      _log('BULK_RESOLVE', smeEmail, 'Bulk resolve', rowIdx, caseData.caseId, remarks.substring(0,80));
  ...
  hideRows();
  return { success: true, message: count + ' cases resolved.' };
}
...
function getAgentLdapList() {
  try {
    ...
    const lastRow = sheet.getLastRow();
    if (lastRow < 2) return [];
    const ldaps = sheet.getRange(2, 2, lastRow - 1, 1).getValues().map(function(r){ return String(r[0]||'').trim(); }).filter(Boolean);
    return Array.from(new Set(ldaps)).sort();
  } catch(e) { return []; }
}
...
var TEAM_SME_MAP = {
  ...
  'Team Gerry':  ['marlynb'],
  'Team Khent':  ['caval'],
  'Team James':  ['criseldaa'],
  'Team Steven': ['sheenamae', 'jgarlet'],
  'Team Jim':    ['glendajoe'],
  'Team Al':     ['mquirol'],
  'Team Faye':   ['neljhon'],
  'Team Mel':    ['mabubay'],
...
var SLA_CC_48HR = ['deanmark@google.com', 'vincentrey@google.com', 'jmontrias@google.com', 'eoliveros@google.com'];
...
function testConfirmationEmails() {
  var ME = 'stevenjosephc@google.com';

  var caseData = {
    submitter     : ME, // route recipient to yourself only
    ldap          : 'testldap',
    caseId        : '7-1624000040947',
    symptom       : 'Refund request - Denied',
    detailedIssue : 'Test detailed issue for confirmation email preview.',
    reason        : 'Test reason for escalation.',
    channel       : 'Chat',
    team          : 'Team Al',
    caseLink      : 'https://cases.connect.corp.google.com/7-1624000040947',
    timestamp     : new Date().toString()
  };

  // ── Test CLAIM email ──
  sendClaimNotification_(caseData, ME);

  // ── Test RESOLVE email ──
  var resolveData = Object.assign({}, caseData, {
    resolutionTime: new Date().toString(),
    remarks       : 'Test SME remarks — validated process and approved refund per policy.'
  });
  sendResolveNotification_(resolveData, ME);

  Logger.log('✅ Test confirmation emails (claim + resolve) sent only to ' + ME);
}
